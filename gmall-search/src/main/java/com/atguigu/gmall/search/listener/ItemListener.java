    ...
    public void listen(Long spuId, Channel channel, Message message) throws IOException {
        ...
        if (!CollectionUtils.isEmpty(skuEntities)){
            ...
            List<Goods> goodsList = skuEntities.stream().map(skuEntity -> {
                ...
                // 通用属性（检索参数）
                ResponseVo<List<SpuAttrValueEntity>> spuAttrResponseVo = this.pmsClient.querySpuAttrValueBySpuId(spuEntity.getId());
                ...
                // 销售属性（检索参数）
                ResponseVo<List<SkuAttrValueEntity>> skuAttrResponseVo = this.pmsClient.selectSkuAttrValueBySkuId(skuEntity.getId());
